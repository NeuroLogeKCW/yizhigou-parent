...
app.controller('contentController', function ($scope, $controller, contentService, contentCategoryService) {
    ...
    $scope.findAllContentCatgoryList = function () {
        contentCategoryService.findAll().success(
            function (response) {
                $scope.contCatgrList = response;
                for (var i = 0; i < response.length; i++) {
                    $scope.contentCatgoryList[response[i].id] = response[i].name;
                }
            }
        )
    }
     ...
     *    前台scope域中需要有容器存放广告
     * 后台返回的是List<Contrent>
     *     $scope.branderList = response
     *     scope.jinrtuijianList = response
     *     $scope.contentList = {categoryId,{listContent}}
     *
     *
     */
    $scope.contentList = [];
    $scope.findByCategoryId = function (categoryId) {
        contentService.findByCategoryId(categoryId).success(
            function (response) {
                ...
                //alert($scope.contentList[categoryId])
            }
        )
    }
